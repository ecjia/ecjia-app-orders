...
// 		$this->db_admin_user		= RC_Loader::load_model('admin_user_model');
	...
	public function back_info() {
		...
		if (!empty($back_id)) {
			...
			$this->showmessage(__('无法找到对应退货单！') , ecjia_admin::MSGTYPE_HTML | ecjia_admin::MSGSTAT_ERROR) ;
		}
		if (empty($back_order)) {
			$this->showmessage(__('无法找到对应退货单！') , ecjia_admin::MSGTYPE_HTML | ecjia_admin::MSGSTAT_ERROR) ;
		}
...
//		}
	...
	public function remove() {
		...
		$this->showmessage(RC_Lang::lang('tips_back_del'), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS,array('pjaxurl'=>RC_Uri::url('orders/admin_order_back/init')));
	}
