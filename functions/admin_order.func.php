...
function order_info($order_id, $order_sn = '', $type = '')
{
    ...
    /* 计算订单各种费用之和的语句 */
    $total_fee = " (goods_amount + tax + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee - discount - bonus - integral_money) AS total_fee ";
...
function get_order_detail($order_id, $user_id = 0, $type = '')
{
    ...
    $order_id   = intval($order_id);
    if ($order_id <= 0) {
        return new ecjia_error('error_order_detail', __('参数无效', 'orders'));
    }
