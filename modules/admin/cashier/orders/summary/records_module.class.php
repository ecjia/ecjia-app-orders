...
/**
 * 资金流水开单/验单列表（支付完成的普通订单）
 * @author zrl
 */
class admin_cashier_orders_summary_records_module extends api_admin implements api_interface
{
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request)
    {
        ...
        if (!empty($type)) {
            $dbview->where(RC_DB::raw('cr.action'), $type);
        }
        ...
        $total_fee    = "(oi.goods_amount + oi.tax + oi.shipping_fee + oi.insure_fee + oi.pay_fee + oi.pack_fee + oi.card_fee - oi.discount - oi.integral_money - oi.bonus) as total_fee";
     ...
     * return array
     */
    private function formated_admin_order_list($data = array())
    {
    	$record_list = [];
        if (!empty($data)) {
