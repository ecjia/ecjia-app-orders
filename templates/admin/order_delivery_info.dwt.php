...
<!-- 									<a href="exchange_goods.php?act=edit&id={$order.extension_id}">{$lang.exchange_goods}</a> -->
				...
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle acc-in" data-toggle="collapse" data-target="#collapseTwo"><strong>{$lang.consignee_info}</strong></a>
					</div>
				...
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle acc-in" data-toggle="collapse" data-target="#collapseThree"><strong>{$lang.goods_info}</strong></a>
					</div>
					<div class="accordion-body in collapse" id="collapseThree">
						<table class="table table-striped m_b0">
							<thead>
								<tr>
									<td><div><strong>{$lang.goods_name_brand}</strong></div></td>
									<td><div><strong>{$lang.goods_sn}</strong></div></td>
									<td><div><strong>{$lang.product_sn}</strong></div></td>
									<td><div><strong>{$lang.goods_attr}</strong></div></td>
									<!-- {if $suppliers_list neq 0} -->
									<td><div><strong>{$lang.suppliers_name}</strong></div></td>
									<!-- {/if} -->
									<td><div><strong>{$lang.storage}</strong></div></td>
									<td><div><strong>{$lang.goods_number}</strong></div></td>
									<td><div><strong>{$lang.goods_delivery}</strong></div></td>
									<td><div><strong>{$lang.goods_delivery_curr}</strong></div></td>
								</tr>
							</thead>
							<tbody>
								...
								<tr>
									<td>
										...
										<a href='{url path="goods/admin/preview" args="id={$goods.goods_id}"}' target="_blank">{$goods.goods_name} {if $goods.brand_name}[ {$goods.brand_name} ]{/if}{if $goods.is_gift}{if $goods.goods_price > 0}{$lang.remark_favourable}{else}{$lang.remark_gift}{/if}{/if}{if $goods.parent_id > 0}{$lang.remark_fittings}{/if}</a>
										<!-- {/if} -->
									</td>
									<td>{$goods.goods_sn}</td>
				...
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle acc-in" data-toggle="collapse" data-target="#collapseFour"><strong>{$lang.action_info}</strong></a>
					</div>
						...
						<table class="table table-oddtd m_b0">
							<tbody class="first-td-no-leftbd">
								...
								<tr>
									...
									<td colspan="3">
										<button class="btn btn-gebo" type="submit" name="delivery_confirmed">{$lang.op_confirm}{$lang.op_split}</button>
										<a href='{url path="orders/admin/info" args="order_id={$order_id}"}'><button class="btn" type="button">{$lang.cancel}</button></a>
