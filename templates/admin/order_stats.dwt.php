...
<div class="alert alert-info">
	...
	<strong>注：</strong>{t}订单统计图表默认显示为按时间段查询{/t}
</div>
<div>
	<h3 class="heading">
		...
		<div class="accordion-body in collapse" id="goods_info_area_submit">
			<table class="table table-oddtd m_b0">
				<tbody class="first-td-no-leftbd">
					<tr>
						<td><div align="right"><strong>{t}有效订单总金额：{/t}</strong></div></td>
						<td>{$order_stats.total_turnover}</td>
						<td><div align="right"><strong>{t}总点击数：{/t}</strong></div></td>
						<td>{$order_stats.click_count}</td>
					</tr>
					<tr>
						<td><div align="right"><strong>{t}每千点击订单数：{/t}</strong></div></td>
						<td>{$order_stats.click_ordernum}</td>
						<td><div align="right"><strong>{t}每千点击购物额：{/t}</strong></div></td>
...
<div class="row-fluid">
	<div class="choose_list f_r">
		<form action="{$form_action}" method="post" name="searchForm">
			<span>按时间段查询：</span>
	  		<input type="text" class="start_date w110" name="start_date" value="{$start_date}" placeholder="开始时间"/>
	  		<span>-</span>
	  		<input type="text" class="end_date w110" name="end_date" value="{$end_date}" placeholder="结束时间"/>
		...
		<div class="span12">
			<div class="tabbable">
				<ul class="nav nav-tabs">
					<li class="{if $page eq 'init'}active{/if}"><a class="data-pjax" href='{url path="orders/admin_order_stats/init"}{if $start_date}&start_date={$start_date}{/if}&end_date={$end_date}&is_multi={$is_multi}{if $year_month}&year_month={$year_month}{/if}'>{t}订单概况{/t}</a></li>
					<li class="{if $page eq 'shipping_status'}active{/if}"><a class="data-pjax" href='{url path="orders/admin_order_stats/shipping_status"}{if $start_date}&start_date={$start_date}{/if}&end_date={$end_date}&is_multi={$is_multi}{if $year_month}&year_month={$year_month}{/if}'>{t}配送方式{/t}</a></li>
					<li class="{if $page eq 'pay_status'}active{/if}"><a class="data-pjax" href='{url path="orders/admin_order_stats/pay_status"}{if $start_date}&start_date={$start_date}{/if}&end_date={$end_date}&is_multi={$is_multi}{if $year_month}&year_month={$year_month}{/if}'>{t}支付方式{/t}</a></li>
				</ul>
