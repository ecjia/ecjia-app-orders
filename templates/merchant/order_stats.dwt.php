								...
								<div class="col-lg-7">
									<div class="row-fluid">
										<table class="table table-striped table-hide-edit">
											...
											<tbody>
												<tr>
													<td><a href="{RC_Uri::url('orders/merchant/init')}" target="__blank">{t domain="orders"}配送型订单{/t}</a></td>
													<td>{$order_stats.order_count_data.order_count}</td>
													<td>{$order_stats.order_count_data.total_fee}</td>
												</tr>
												<tr>
													<td><a href="{RC_Uri::url('orders/merchant/init')}&extension_code=group_buy" target="__blank">{t domain="orders"}团购型订单{/t}</a></td>
													<td>{$order_stats.groupbuy_count_data.order_count}</td>
													<td>{$order_stats.groupbuy_count_data.total_fee}</td>
												</tr>
												<tr>
													<td><a href="{RC_Uri::url('orders/merchant/init')}&extension_code=storebuy" target="__blank">{t domain="orders"}到店型订单{/t}</a></td>
													<td>{$order_stats.storebuy_count_data.order_count}</td>
													<td>{$order_stats.storebuy_count_data.total_fee}</td>
												</tr>
												<tr>
													<td><a href="{RC_Uri::url('orders/merchant/init')}&extension_code=storepickup" target="__blank">{t domain="orders"}自提型订单{/t}</a></td>
													<td>{$order_stats.storepickup_count_data.order_count}</td>
													<td>{$order_stats.storepickup_count_data.total_fee}</td>
												</tr>
												<tr>
													<td><a href="{RC_Uri::url('orders/merchant/init')}&extension_code=storepickup" target="__blank">{t domain="orders"}收银台型订单{/t}</a></td>
