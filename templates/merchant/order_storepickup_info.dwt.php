			...
			<div class="accordion-group panel panel-default">
                ...
                <div class="accordion-body in collapse " id="collapsePickup">
					<table class="table table-oddtd m_b0">
						<tbody class="first-td-no-leftbd">
							<tr>
								<td><div align="right"><strong>{t domain="orders"}提货码：{/t}</strong></div></td>
								...
								<td><div align="right"><strong>{t domain="orders"}提货状态：{/t}</strong></div></td>
								<td>{if $pickup_status}{$pickup_status}{else}{t domain="orders"}暂无{/t}{/if}</td>
							</tr>
							<tr>
								<td><div align="right"><strong>{t domain="orders"}预约提货时间：{/t}</strong></div></td>
								<td colspan="3">
									{if $order.expect_shipping_time} {$order.expect_shipping_time} {else} {t domain="orders"}暂无{/t} {/if}
								</td>
							</tr>
						</tbody>
					</table>
                	...
                	<table class="table table-oddtd m_b0">
						<tbody class="first-td-no-leftbd">
							...
							<tr>
								...
								<td>{if $inv_payee}{$inv_payee}{else}{t domain="orders"}个人{/t}{/if}</td>
                	...
                	<table class="table table-oddtd m_b0">
						<tbody class="first-td-no-leftbd">
							...
							<tr>
								...
								<td colspan="3">
									<a class="btn btn-info confirm_validate" data-url='{url path="orders/mh_validate_order/validate_to_ship"}' data-refresh='{url path="orders/merchant/info"}&order_id={$order.order_id}'>{t domain="orders"}确认验证{/t}</a>
							...
							<tr>
								...
								<td colspan="3">
                                    {t domain="orders"}【确认验证】设置该笔订单的商品已被买家取走；{/t}<br>
								</td>
							</tr>
						</tbody>
					</table>
                	...
                	<table class="table table-oddtd m_b0">
						<tbody class="first-td-no-leftbd">
							...
							<tr>
								<td width="15%"><div align="right"> <strong>{t domain="orders"}操作说明：{/t}</strong></div></td>
								<td colspan="3">
                                    {t domain="orders"}【取消】设置该订单为无效/作废订单；{/t}<br>
								</td>
							</tr>
						</tbody>
					</table>
