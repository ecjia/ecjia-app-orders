	    ...
	    <div class="panel">
			...
			<div class='panel-body panel-body-small'>
        		<ul class="nav nav-pills pull-left">
					<li class="{if $composite_status eq '' || !$composite_status}active{/if}"><a class="data-pjax" href='{RC_Uri::url("orders/merchant/today_order")}{if $smarty.get.keywords}&keywords={$smarty.get.keywords}{/if}'>全部 <span class="badge badge-info">{if $count.all}{$count.all}{else}0{/if}</span></a></li>
					<li class="{if $composite_status eq 'await_confirm'}active{/if}"><a class="data-pjax" href='{RC_Uri::url("orders/merchant/today_order")}&composite_status=0{if $smarty.get.keywords}&keywords={$smarty.get.keywords}{/if}'>待确认<span class="badge badge-info">{if $count.unconfirmed}{$count.unconfirmed}{else}0{/if}</span></a></li>
					<li class="{if $composite_status eq 'await_pay'}active{/if}"><a class="data-pjax" href='{RC_Uri::url("orders/merchant/today_order")}&composite_status=100{if $smarty.get.keywords}&keywords={$smarty.get.keywords}{/if}'>待付款<span class="badge badge-info">{if $count.await_pay}{$count.await_pay}{else}0{/if}</span></a></li>
					<li class="{if $composite_status eq 'await_ship'}active{/if}"><a class="data-pjax" href='{RC_Uri::url("orders/merchant/today_order")}&composite_status=101{if $smarty.get.keywords}&keywords={$smarty.get.keywords}{/if}'>待发货<span class="badge badge-info">{if $count.await_ship}{$count.await_ship}{else}0{/if}</span></a></li>
					<li class="{if $composite_status eq 'order_finished'}active{/if}"><a class="data-pjax" href='{RC_Uri::url("orders/merchant/today_order")}&composite_status=102{if $smarty.get.keywords}&keywords={$smarty.get.keywords}{/if}'>已完成<span class="badge badge-info">{if $count.finished}{$count.finished}{else}0{/if}</span></a></li>
				</ul>
